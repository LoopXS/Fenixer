...
async def get_chatbot_reply(message):
    ...
    req_link = chatbot_base.format(
        ultroid_bot.me.first_name or "heartless user",
        message,
    )
    try:
...
def telegraph_client():
    ...
    try:
        ...
        if "SHORT_NAME_TOO_LONG" in str(er):
            TelegraphClient.create_account(
                short_name="heartless_user", author_name=gd_name, author_url=profile_url
            )
        else:
            LOGS.exception(er)
            return
...
async def Carbon(
    ...
    **kwargs,
    ...
    if rayso:
        ...
        kwargs["title"] = kwargs.get("title", "heartless")
    else:
        kwargs["code"] = code
