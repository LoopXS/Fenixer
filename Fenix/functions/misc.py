...
from Fenix.exceptions import DependencyMissingError
...
async def allcmds(event, telegraph):
    ...
    for z in LIST.keys():
        ...
        for zz in LIST[z]:
            txt += HNDLR + zz + "\n"
        txt += "\n\n"
    t = telegraph.create_page(title="UserBot All Cmds", content=[txt])
    await eor(event, f"All Cmds : [Click Here]({t['url']})", link_preview=False)
