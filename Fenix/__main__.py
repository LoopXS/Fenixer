...
def main():
    ...
    if (
        ...
        os.execl(sys.executable, "python3", "-m", "Fenix")
    ...
    suc_msg = """
            ----------------------------------------------------------------------
                UserBotX has been deployed! Visit @PentesterX for updates!!
            ----------------------------------------------------------------------
    """
    ...
    LOGS.info(
        f"Took {time_formatter((time.time() - start_time)*1000)} to start •UserBot•"
    )
    LOGS.info(suc_msg)
