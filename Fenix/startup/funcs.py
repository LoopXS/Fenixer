...
async def autobot():
    ...
    if who.username:
        ...
        username = "PentesterX_" + (str(who.id))[5:] + "_bot"
    bf = "@BotFather"
    ...
    if not isdone.startswith("Good."):
        await ultroid_bot.send_message(bf, "𝒂𝒔𝒔𝒊𝒔𝒕𝒂𝒏𝒕")
        await asyncio.sleep(1)
        isdone = (await ultroid_bot.get_messages(bf, limit=1))[0].text
        if not isdone.startswith("Good."):
            LOGS.critical(
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "PentesterX_" + (str(who.id))[6:] + str(ran) + "_bot"
        await ultroid_bot.send_message(bf, username)
        await asyncio.sleep(1)
...
async def customize():
    ...
    try:
        ...
        await ultroid_bot.send_message(
            "botfather", f"➰ ᥱ᥊ᥴᥣᥙsi᥎ᥱ ᥴ᧐ⲙⲙᥙᥒiᥴᥲᴛi᧐ᥒ δ᧐ᴛ\n\n🎭 ɗᥱ᥎ᥱᥣ᧐ρᥱɗ δy ɦᥱᥲrᴛᥣᥱss\n\n✨ T.me/DarkPentester"
        )
        await asyncio.sleep(2)
        ...
        try:
            async for x in ultroid_bot.iter_messages(
                ...
                try:
                    ...
                    LOGS.info(f"Heartless - PLUGIN_CHANNEL - ERROR - {plugin}")
                    LOGS.exception(e)
                    os.remove(plugin)
...
• Here, are the Some Basic stuff from, where you can Know, about its Usage."""
        ...
        MSG = f"**Fenix has been deployed!**\n➖➖➖➖➖➖➖➖➖➖\n**UserMode**: {inline_mention(ultroid_bot.me)}\n**Assistant**: @{asst.me.username}\n➖➖➖➖➖➖➖➖➖➖\n**Support**: @DarkPentesterX\n➖➖➖➖➖➖➖➖➖➖"
